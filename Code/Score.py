...
class Score:
    def __init__(self, window):  # não está reconhecendo o Surface
        self.window = window
    ...
    def show(self):
        ...
        db_proxy.close()
        for player_sore in list_score:
            id_, name, score,  date = player_sore
            self.score_text(20, f'{name}     {score:05d}     {date}', COLOR_YELLOW, SCORE_POS[list_score.index(player_sore)])
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
